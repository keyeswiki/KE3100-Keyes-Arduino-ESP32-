...
将我们提供的代码文件压缩包解压，把解压后的代码文件夹保存到方便使用的位置。我们提供本课程需要用到的所有代码文件，并保存到方便使用的位置。本课程以代码存放于D盘为例，路径为 <u>D:KidsBlock(Scratch)资料\KidsBlock(Scratch)_代码</u>。（你也可以放入其他磁盘，只要方便示例代码导入就行）
